...
using namespace std;
//обеспеч
//приостанавливает выполнение программы клиента до момента, пока программа клиента не выполнит функцию CreateFile.
int main()
{
	setlocale(LC_ALL, "ru");
	HANDLE sH;	// дескриптор канала(возвращаемым значением)
	DWORD lp;
	char buf[50];
	...
	try
	{
		...
		if (!ConnectNamedPipe(sH, NULL))        // подсоединение сервера к каналу  
