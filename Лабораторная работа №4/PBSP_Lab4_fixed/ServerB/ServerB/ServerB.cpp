...
bool GetRequestFromClient(char* name, short port, struct sockaddr* from, int* flen)                                                                 //позывной                    //Функция предназначена для обработки запроса клиентской программы
                                                                                                                                                                     //[позывной сервера, номер прослушиваемого порта, указатель на структуру SOCKADDR_IN, указатель на размер from]
{
    int lbuf;
    char ibuf[50] = "";
    ...
    while (true)
    {
        if (lbuf = recvfrom(port, ibuf, sizeof(ibuf), NULL, from, flen) == SOCKET_ERROR)                       //ожидание запроса в функции..... осуществляется
        {                                                                                                      //если поступившее сообщение явл                                                               
...
bool PutAnswerToClient(char* name, short port, struct sockaddr* to, int* lto)                                                                                   //Функция оправляет в адрес клиента  свой позывной, что предполагает готовность сервера  к дальнейшей работе с клиентом.                                                                                       //Функция предназначена для подтверждения сервером запроса на установку соединения
{
    int lbuf;
